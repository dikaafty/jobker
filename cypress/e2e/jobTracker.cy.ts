describe("Job Tracker", () => {
  ...
  it("should edit a job", () => {
    ...
    cy.findByLabelText(/status/i).select(job.newStatus).should("have.value", job.newStatus);

    cy.contains("Save Job").click();
    cy.get("[data-testid='dialog']").should("not.exist");
    cy.get("tbody td").contains(job.status).should("not.exist");
    cy.get("tbody td").contains(job.newStatus).should("be.visible");
  });
});